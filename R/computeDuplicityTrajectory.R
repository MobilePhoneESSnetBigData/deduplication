...
computeDuplicityTrajectory <-function(path, devices, gridParams, pairs, P1 , T) {
  ...
  centerOfProbs <- list(length = ndevices)
  dr<-list(length = ndevices)
  ...
  # res <- clusterApplyLB( cl, ichunks, doCPP, centrs, PostLoc )
  # for(i in 1:length(res)) {
  #   cpp <- c(cpp, res[[i]]) 
  # }
  ...
  # res<-clusterApplyLB(cl, ichunks, doPairs, pairs, dr, T, cpp)
  # pd1d2 <- NULL
  # for (i in 1:length(res)) {
...
#   for ( i in ichunks) {
#      local_cpp[[i]]<-buildCentroidProbs(centrs, postLoc, ichunks[[i]])
#   }
#   return local_cpp
# }
#
# doPairs <- function(ichunks, pairs, ndev, dr, T) {
#     ll.matrix <- matrix(0L, nrow = ndev, ncol = ndev)
#      n <- length(ichunks)
#      for( i in 1:n ) {
#        
#      }
