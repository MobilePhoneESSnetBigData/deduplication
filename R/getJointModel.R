...
#'
#' @description Builds the joint HMM model using the emmission probabilities 
#' given by \code{getEmissionProbsJointModel()}.
#'
#' @param nrows Number of rows in the grid.
#'
#' @param ncols Number of columns in the grid.
#'
#' @param jointEmissionProbs A (sparse) matrix with the joint event location 
...
#'
#' @param initSteady If TRUE the initial apriori distribution is set to the 
#' steady state of the transition matrix, if FALSE the apriori distribution 
#' should be given as a parameter.
#'
#' @param aprioriJointProb The apriori distribution for the HMM model. It is 
#' needed only if initSteady is FALSE.
#'
#' @return Returns an HMM model with the initial apriori distribution set to the 
#' steady state of the transition matrix or to the value given by 
#' \code{aprioriJointProb} parameter.
#'
#' @import destim
#' @export
getJointModel <-
