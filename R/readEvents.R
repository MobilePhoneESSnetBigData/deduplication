...
#'
#' @return Returns a data.table object that contains the events generated by the mobile network. The number of rows equls the number
#' of connection events recorded by the network. The returned object has the following columns: time, deviceID, eventCode, antennaID,
#' x, y, tile, obsVar. 
#' 
#' 
#' @import data.table
#'
#' @export
readEventsFile <- function(eventsFileName, simulatedData = TRUE) {
  
  if(simulatedData == TRUE) {
      
    events.dt <- fread(eventsFileName, sep = ',', stringsAsFactors = FALSE, header = TRUE,
    ...
    events.dt_noDup <- copy(events.dt)[, list(eventCode = as.character(min(as.numeric(eventCode)))), by = c("time", "deviceID")]
    events.dt <- merge(events.dt_noDup, events.dt, by = names(events.dt_noDup), all.x = TRUE)
    events.dt <- events.dt[!duplicated(events.dt, by = c("time", "deviceID", "eventCode"))]
